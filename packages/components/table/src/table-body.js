...
export default {
			...
			<table
				...
				<tbody>
					...
					<tu-tooltip
						...
						placement="top"
						ref="tooltip"
						content={this.tooltipContent}></tu-tooltip>
				</tbody>
			</table>
		);
	},
	...
	watch: {
		"store.states.hoverRow"(newVal, oldVal) {
			...
			raf(() => {
				const rows = this.$el.querySelectorAll(".tu-table__row");
	...
	methods: {
		...
		getRowClass(row, rowIndex) {
			const classes = ["tu-table__row"];
			if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
				classes.push("current-row");
			}

			if (this.stripe && rowIndex % 2 === 1) {
				classes.push("tu-table__row--striped");
			}
		...
		handleCellMouseEnter(event, row) {
			...
			if (!(hasClass(cellChild, "tu-tooltip") && cellChild.childNodes.length)) {
				return;
			}
					...
					[
						...
						<tr key={"expanded-row__" + tr.key}>
							<td
								...
								class="tu-table__cell tu-table__expanded-cell">
