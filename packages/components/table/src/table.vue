<template>
	<div
		...
		:class="[
			{
				'tu-table--fit': fit,
				'tu-table--striped': stripe,
				'tu-table--border': border || isGroup,
				'tu-table--hidden': isHidden,
				'tu-table--group': isGroup,
				'tu-table--fluid-height': maxHeight,
				'tu-table--scrollable-x': layout.scrollX,
				'tu-table--scrollable-y': layout.scrollY,
				'tu-table--enable-row-hover': !store.states.isComplex,
				'tu-table--enable-row-transition':
	...
	>
		...
		<div
			...
			class="tu-table__header-wrapper"
			ref="headerWrapper"
		>
			<table-header
				ref="tableHeader"
		...
		>
			...
			<div
				...
				class="tu-table__empty-block"
				ref="emptyBlock"
				:style="emptyBlockStyle"
			>
				<span class="tu-table__empty-text">
					<slot name="empty">{{ emptyText || t("tu.table.emptyText") }}</slot>
				</span>
			</div>
			<div
				v-if="$slots.append"
				class="tu-table__append-wrapper"
				ref="appendWrapper"
			>
		...
		<div
			...
			v-mousewheel="handleHeaderFooterMousewheel"
			class="tu-table__footer-wrapper"
			ref="footerWrapper"
		>
			<table-footer
				:store="store"
				:border="border"
				:sum-text="sumText || t('tu.table.sumText')"
		...
		>
			<div
				v-if="showHeader"
				class="tu-table__fixed-header-wrapper"
			...
			<div
				class="tu-table__fixed-body-wrapper"
			...
			<div
				v-if="showSummary"
				v-show="data && data.length > 0"
				class="tu-table__fixed-footer-wrapper"
		...
		<div
			v-if="rightFixedColumns.length > 0"
			class="tu-table__fixed-right-patch"
			ref="rightFixedPatch"
			:style="{
		...
		></div>
		<div
			class="tu-table__column-resize-proxy"
			ref="resizeProxy"
...
<script>
	import { debounce, throttle } from "@packages/src/utils/throttle-debounce";
	import { addResizeListener, removeResizeListener } from "@packages/src/utils/resize-event";
	import Mousewheel from "@packages/src/directives/mousewheel";
	...
	export default {
		...
		created() {
			this.tableId = "tu-table_" + tableIdSeed++;
		...
		methods: {
			...
			syncPostion() {
				...
				const { headerWrapper, footerWrapper, fixedBodyWrapper, rightFixedBodyWrapper } =
					this.$refs;
