...
import Menubar from "@packages/src/utils/menu/aria-menubar";
import { addClass, removeClass, hasClass } from "@packages/src/utils/dom";

export default {
  name: "TuMenu",

  componentName: "TuMenu",
  ...
  watch: {
    ...
    collapse(value) {
      ...
      this.broadcast("TuSubmenu", "toggle-collapse", value);
    },
  },

  components: {
    "tu-menu-collapse-transition": {
      functional: true,
      render(createElement, context) {
        const data = {
          ...
          on: {
            ...
            enter(el) {
              addClass(el, "tu-opacity-transition");
              el.style.opacity = 1;
            },

            afterEnter(el) {
              removeClass(el, "tu-opacity-transition");
              el.style.opacity = "";
            },
            ...
            beforeLeave(el) {
              ...
              if (hasClass(el, "tu-menu--collapse")) {
                removeClass(el, "tu-menu--collapse");
                el.dataset.oldOverflow = el.style.overflow;
                el.dataset.scrollWidth = el.clientWidth;
                addClass(el, "tu-menu--collapse");
              } else {
                addClass(el, "tu-menu--collapse");
                el.dataset.oldOverflow = el.style.overflow;
                el.dataset.scrollWidth = el.clientWidth;
  ...
  render(h) {
    const component = (
      <ul
        ...
        class={{
          "tu-menu--horizontal": this.mode === "horizontal",
          "tu-menu--collapse": this.collapse,
          "tu-menu": true,
        }}
      >
        {this.$slots.default}
    ...
    if (this.collapseTransition) {
      return (
        <tu-menu-collapse-transition>{component}</tu-menu-collapse-transition>
      );
    } else {
      return component;
    }
