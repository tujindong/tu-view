<template>
	...
	>
		...
		<span
			...
			class="tu-input__prefix"
		>
			<span class="tu-input__prefix-inner">
				<span
					v-if="$slots.prefix"
					class="tu-input__icon-slot"
				>
		...
		<span class="tu-input__suffix">
			<span class="tu-input__suffix-inner">
				...
				<span
					...
					class="tu-input__count"
				>
					<span class="tu-input__count-inner"> {{ textLength }}/{{ upperLimit }} </span>
				</span>
			</span>
		</span>
	</div>
...
<script>
	...
	export default {
		...
		computed: {
			...
			inputType() {
				return this.showPassword ? (this.isPasswordVisible ? "text" : "password") : this.type;
			},
