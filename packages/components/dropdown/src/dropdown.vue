...
export default {
  ...
  watch: {
    visible(val) {
      this.broadcast("TuDropdownMenu", "visible", val);
      this.$emit("visible-change", val);
    },

    focusing(val) {
      const selfDefine = this.$el.querySelector(".tu-dropdown-selfdefine");
      if (selfDefine) {
        // 自定义
        if (val) {
  ...
  render(h) {
    ...
    return (
      <div class="tu-dropdown" v-clickoutside={hide} aria-disabled={disabled}>
        {triggerElm}
        {menuElm}
      </div>
    );
