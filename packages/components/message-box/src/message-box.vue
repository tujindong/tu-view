	...
	export default {
		...
		computed: {
			icon() {
				...
				return iconClass || (type && typeMap[type] ? `tu-icon-${typeMap[type]}` : "");
			},
		...
		methods: {
			...
			validate() {
				if (this.$type === "prompt") {
					...
					if (inputPattern && !inputPattern.test(this.inputValue || "")) {
						this.editorErrorMessage = this.inputErrorMessage || t("tu.messagebox.error");
			...
			getFirstFocus() {
				...
				const title = this.$el.querySelector(".tu-message-box__btns .tu-message-box__title");
				return btn || title;
			},
			...
			setBackgroundColor() {
				const el = this.$refs.messageboxWrapper;
				const siblingsEl = el.parentElement.children[0];
				const color = siblingsEl ? getBackground(siblingsEl) : "#ffffff";
				this.$refs.messagebox.style.backgroundColor = color;
			},
		},
	};
</script>
