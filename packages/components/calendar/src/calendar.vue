<template>
  <div :class="['tu-calendar', type ? `tu-calendar--${type}` : '']">
    <div class="tu-calendar__header">
      ...
      <div class="tu-calendar__button-group" v-if="validatedRange.length === 0">
        <tu-button-group>
          <tu-button size="mini" @click="selectDate('prev-month')">
            {{ t("tu.datepicker.prevMonth") }}
          </tu-button>
          <tu-button size="mini" @click="selectDate('today')">
            {{ t("tu.datepicker.today") }}
          </tu-button>
          <tu-button size="mini" @click="selectDate('next-month')">
    ...
    <div v-else class="tu-calendar__body" key="has-range">
      <date-table
...
export default {
  ...
  computed: {
    ...
    currentDate() {
      ...
      return `${year} ${this.t("tu.datepicker.year")} ${this.t(
        "tu.datepicker.month" + month
      )}`;
    },
    ...
    validatedRange() {
      ...
      if (range.length === 2) {
        ...
        if (start > end) {
          console.warn(
            "[TuViewCalendar]end time should be greater than start time"
          );
          return [];
        }
