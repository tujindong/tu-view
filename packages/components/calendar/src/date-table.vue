<script>
	...
	export default {
		...
		computed: {
			...
			formatedToday() {
				return this.tuCalendar.formatedToday;
			},
		...
		methods: {
			...
			cellRenderProxy({ text, type }) {
				let render = this.tuCalendar.$scopedSlots.dateCell;
				...
				<table
					...
					<tbody>
						{this.rows.map((row, index) => (
							<tr
								class={{
									"tu-calendar-table__row": true,
									"tu-calendar-table__row--hide-border": index === 0 && this.hideHeader,
								}}
								key={index}>
								{row.map((cell, key) => (
									<td
										...
										<div class="tu-calendar-day">{this.cellRenderProxy(cell)}</div>
									</td>
								))}
							</tr>
						))}
