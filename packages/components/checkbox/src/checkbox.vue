<template>
  <label
    class="tu-checkbox"
    :class="[
      (isBorder || isButton) && checkboxSize
        ? `tu-checkbox--${checkboxSize}`
        : '',
  ...
  >
    ...
    >
      <span v-if="!isButton" class="tu-checkbox__inner"></span>
      <input
...
export default {
  ...
  computed: {
    ...
    isBorder() {
      return this.isGroup
        ? this._checkboxGroup.border || this.border
        : this.border;
    },

    isButton() {
      return this.isGroup
        ? this._checkboxGroup.button || this.button
        : this.button;
    },
