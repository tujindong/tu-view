<template>
  <span ref="popperWrapper">
    ...
    >
      <div
        class="tu-popover tu-popper"
        :class="[popperClass, content && 'tu-popover--plain']"
        ref="popper"
      ...
      >
        <div class="tu-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
    <span class="tu-popover__reference-wrapper" ref="wrapper">
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, "tu-popover__reference");
    ...
    if (this.trigger === "click") {
      ...
      if (this.tabindex < 0) {
        console.warn(
          "[TuView Warn][Popover]a negative taindex means that the element cannot be focused by tab key"
        );
      }
