	...
	>
		...
		>
			...
			<template slot="suffix">
				<i
					v-if="loading"
					class="tu-select__caret tu-icon-reload"
				></i>
				<template v-if="!loading">
					<i
						v-show="!showClose"
						:class="['tu-select__caret', 'tu-input__icon', `tu-icon-${iconDirection}`]"
					></i>
					<i
		...
		>
			...
			>
				...
				<template v-if="emptyText">
					...
					></slot>
					<p
						class="tu-select-dropdown__empty"
						v-else
					>
						{{ emptyText }}
					</p>
	...
	export default {
		...
		computed: {
			...
			showClose() {
				...
				return this.clearable && this.inputHovering && hasValue && !this.selectDisabled;
			},
			...
			propPlaceholder() {
				return typeof this.placeholder !== "undefined" ? this.placeholder : "请输入";
			},
		},
		...
		methods: {
			...
			scrollToOption(option) {
				...
				if (this.$refs.popper && target) {
					const menu = this.$refs.popper.$el.querySelector(".tu-select-dropdown__wrap");
