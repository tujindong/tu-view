...
import { isFunction } from "@packages/src/utils/types";
export default {
	name: "TuDescriptions",
	...
	computed: {
		descriptionsSize() {
			return this.size || (this.$TUVIEW || {}).size;
		},
	},
		...
		getRows() {
			const children = (this.$slots.default || []).filter(
				vnode =>
					...
					vnode.componentOptions.Ctor.options.name === "TuDescriptionsItem"
			);
	...
	render() {
		...
		return (
			<div class="tu-descriptions">
				{title || extra || $slots.title || $slots.extra ? (
					<div class="tu-descriptions__header">
						<div class="tu-descriptions__title">{$slots.title ? $slots.title : title}</div>
						<div class="tu-descriptions__extra">{$slots.extra ? $slots.extra : extra}</div>
					</div>
				) : null}

				<div class="tu-descriptions__body">
					<table
						class={[
							"tu-descriptions__table",
							{ "is-bordered": border },
							descriptionsSize ? `tu-descriptions--${descriptionsSize}` : "",
						]}>
						{rows.map(row => (
