<template>
	<div style="width: 100%">
		...
		>
			...
			<i
				slot="suffix"
				class="tu-input__icon tu-input__icon--suffix"
		...
		<div
			class="tu-date-editor tu-range-editor tu-input__inner"
			:class="[
				'tu-date-editor--' + type,
			...
			v-else
		>
			<i :class="['tu-input__icon', 'tu-range__icon', triggerClass]"></i>
			<input
				autocomplete="off"
				...
				@focus="handleFocus"
				class="tu-range-input"
			/>
			<slot name="range-separator">
				<span class="tu-range-separator">{{ rangeSeparator }}</span>
			</slot>
			<input
				autocomplete="off"
			...
			<i
				...
				v-if="haveTrigger"
				:class="[showClose ? '' + clearIcon : '']"
				class="tu-input__icon tu-range__close-icon"
			>
			</i>
		</div>
	</div>
...
<script>
	...
	import merge from "@packages/src/utils/merge";
	...
	export default {
		...
		props: {
			...
			clearIcon: {
				type: String,
				default: "tu-icon-close-circle-fill",
			},
			name: {
				default: "",
				validator,
		...
		computed: {
			...
			triggerClass() {
				return (
					this.prefixIcon ||
					(this.type.indexOf("time") !== -1 ? "tu-icon-time-circle" : "tu-icon-calendar")
				);
			},
			...
			pickerVisible(val) {
				...
				if (val) {
					...
					if (this.validateEvent) {
						this.dispatch("TuFormItem", "tu.form.blur");
					}
					this.$emit("blur", this);
					this.blur();
				}
		...
		methods: {
			...
			mountPicker() {
				...
				this.picker.setBackgroundColor(this.reference);
